...
exports.getCocktail = async (req, res) => {
    ...
    try {
        ...
        let cocktail = await Cocktail.findOne({ where: { id: cocktailId }, include: { model: User, attributes: ['id', 'email'] } })
...
exports.addCocktail = async (req, res) => {
    ...
    const user_id = req.ident
    const newCocktail = req.file ? {
        ...req.body,...req.ident, imageUrl: `${req.protocol}://${req.get("host")}/images/${req.file.filename}`
    } : {
        ...req.body,...req.ident
    }
    ...
    try {
        ...
        // Céation du cocktail
        cocktail = await Cocktail.create(newCocktail)
...
exports.updateCocktail = async (req, res) => {
    ...
    try {
        ...
        let imageUrl = cocktail.imageUrl

        if (req.file) {
            const filename = cocktail.imageUrl.split("/images/")[1]
        ...
        await Cocktail.update({ ...req.body, imageUrl: imageUrl }, { where: { id: cocktailId } })
...
exports.deleteCocktail = async (req, res) => {
    ...
    try{
        let cocktail = await Cocktail.findOne({ where: { id: cocktailId }, raw: true })
        let filename = cocktail.imageUrl.split('/').pop()
        fs.unlink(`images/${filename}`, (err) => {
            if (err) {
                console.log(err)
            } else {
